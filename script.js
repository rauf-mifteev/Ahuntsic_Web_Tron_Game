...
class TronGame {
    constructor(canvasId) {
        ...
        this.result = null; // "player1" | "player2" | "draw"
    ...
    update() {
        ...
        // Collision frontale : les deux visent la même cellule vide
        const headOn = (next1.x === next2.x && next1.y === next2.y);

        if ((p1crashes && p2crashes) || headOn) {
            // Match nul : les deux meurent simultanément
            this.player1.die();
            this.player2.die();
            this.result = "draw";
        } else if (p1crashes) {
    ...
    drawResult() {
        const messages = {
            player1: "JOUEUR 1 GAGNE !",
            player2: "JOUEUR 2 GAGNE !",
            draw:    "MATCH NUL !"
        };
        ...
        this.ctx.fillText(messages[this.result], this.canvas.width / 2, this.canvas.height / 2);
    }
